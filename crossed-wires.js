...
const firstWire = processWire(wire1);
const secondWire = processWire(wire2);
const intersections = findIntersection(firstWire, secondWire);
fewestSteps(firstWire, secondWire, intersections);

function fewestSteps(wire1, wire2, intersections) {
  const steps = intersections.map(i => {
    const wire1Steps = calcSteps(wire1, i);
    const wire2Steps = calcSteps(wire2, i);
    return wire1Steps + wire2Steps;
  });
  const fewestSteps = Math.min(...steps);
  console.log(fewestSteps);
}

function shortestDist(intersections) {
  const shortestDist = Math.min(...intersections.map(i => calcDistance(i)));
  console.log(shortestDist);
}

function calcSteps(wire, intersection) {
  let counter = 0;
  for(let i = 0; i < wire.length; i++) {
    const c = wire[i];
    counter++;
    if(isSameCoord(c, intersection)) {
      return counter;
    }
  }
}
...
function findIntersection(wire1, wire2) {
  const intersections = [];
  for(const c1 of wire1) {
    for(const c2 of wire2) {
      if(isSameCoord(c1, c2)) {
...
function processWire(wire) {
  let coords = [];
  let lastCoords = {x: 0, y: 0};
  for(const i of wire) {
    const instructionCoords = instructionToCoords(lastCoords, i);
    lastCoords = instructionCoords[instructionCoords.length - 1];
    coords = coords.concat(instructionCoords);
  }
  return coords;
}

function instructionToCoords(startCoords, instruction) {
  ...
  for(let i = 0; i < distance; i++) {
    if(letter === 'U') {
      lastCoords = moveUp(lastCoords);
    } else if(letter === 'D') {
      lastCoords = moveDown(lastCoords);
    } else if(letter === 'R') {
      lastCoords = moveRight(lastCoords);
    } else if(letter === 'L') {
      lastCoords = moveLeft(lastCoords);
    } else {
      throw new Error('undefined direction', instruction);
    }
...
function moveUp(startCoords) {
  return {x: startCoords.x, y: startCoords.y + 1};
}

function moveDown(startCoords) {
  return {x: startCoords.x, y: startCoords.y - 1};
}

function moveRight(startCoords) {
  return {x: startCoords.x + 1, y: startCoords.y};
}

function moveLeft(startCoords) {
  return {x: startCoords.x - 1, y: startCoords.y};
}

function isSameCoord(c1, c2) {
